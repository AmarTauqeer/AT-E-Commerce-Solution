...
import { productDelete } from "@/app/services/product";
import { permissionDelete } from "@/app/services/user-permissions";

export type typeId = {
  id: string
}

export function DeletePermissionDialog(id: typeId) {
  ...
  const handleDelete = async (id: typeId) => {
    setLoading(true);
    const response = await permissionDelete({ 'id': id })
